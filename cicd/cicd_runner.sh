...
docker rm splunk --force
#docker  rm cypress_runner --force
...
#docker cp $CI_PROJECT_DIR≈ $container:$SPLUNK_ETC/system/local/inputs.conf
# Copy in the sample app
docker cp $CI_PROJECT_DIR/$APP_ROOT $container:$APPS_DIR/$APP_ROOT
docker cp $CI_PROJECT_DIR/cicd/config/inputs.conf $container:$APPS_DIR/$APP_ROOT/default/inputs.conf
...
if [[ $health == "" ]]; then
  ...
  echo -e "\n\033[0;32m\xE2\x9C\x94\033[0m Splunk Available! now go weiter"
fi

echo "und raus aus der schleife"
...
# echo  "sleeping for another 20 seconds, cause sleeping is fine.."
# sleep 20

eventCount=`docker exec $container bash -c "SPLUNK_USERNAME=admin SPLUNK_PASSWORD=newPassword /opt/splunk/bin/splunk search 'index=main sourcetype=routeros tag=authentication action=failure earliest=-24h@h | stats count' -app search"`
if [[ $eventCount =~ "10" ]]; then
    echo -e "\n\033[0;32m\xE2\x9C\x94\033[0m test successful!"
    splunkReady=1
else
  echo -e "apptest failed...output: $eventCount"
  exit 1
fi
